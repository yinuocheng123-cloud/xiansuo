...
- `/tasks/:id`
  任务详情页，展开当前候选页状态、失败样本明细、重试记录明细、最近失败趋势、最近失败原因分类、失败原因 Top 摘要、按原因的最近变化提示、最近成功入池趋势和该任务最近线索
- `/results`
...
- 任务详情页、审核状态筛选、省市筛选、来源平台+线索类型筛选已通过 `EXPLAIN` 复核，当前能命中对应组合索引
- 品牌模糊筛选已改为 `brand ilike`，避免 `coalesce(brand, '')` 破坏 `idx_leads_brand_trgm` 的参与机会
